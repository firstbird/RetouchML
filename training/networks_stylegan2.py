...
def get_weight(shape, gain=1, use_wscale=True, lrmul=1, weight_var='weight'):
    ...
    return tf.compat.v1(weight_var, shape=shape, initializer=init) * runtime_coef
...
def apply_bias_act(x, act='linear', alpha=None, gain=None, lrmul=1, bias_var='bias'):
    b = tf.compat.v1(bias_var, shape=[x.shape[1]], initializer=tf.initializers.zeros()) * lrmul
...
def G_main(
    ...
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
    ...
    # Setup variables.
    lod_in = tf.compat.v1('lod', initializer=np.float32(0), trainable=False)
    dlatent_avg = tf.compat.v1('dlatent_avg', shape=[dlatent_size], initializer=tf.initializers.zeros(), trainable=False)
...
def G_synthesis_stylegan_revised(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    lod_in = tf.cast(tf.compat.v1('lod', initializer=np.float32(0), trainable=False), dtype)
    ...
    for layer_idx in range(num_layers - 1):
        ...
        noise_inputs.append(tf.compat.v1('noise%d' % layer_idx, shape=shape, initializer=tf.initializers.random_normal(), trainable=False))
    ...
    def layer(x, layer_idx, fmaps, kernel, up=False):
        ...
        noise_strength = tf.compat.v1('noise_strength', shape=[], initializer=tf.initializers.zeros())
    ...
    with tf.variable_scope('4x4'):
        with tf.variable_scope('Const'):
            x = tf.compat.v1('const', shape=[1, nf(1), 4, 4], initializer=tf.initializers.random_normal())
...
def D_stylegan(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    labels_in = tf.cast(labels_in, dtype)
    lod_in = tf.cast(tf.compat.v1('lod', initializer=np.float32(0.0), trainable=False), dtype)
