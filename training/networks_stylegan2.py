...
def get_weight(shape, gain=1, use_wscale=True, lrmul=1, weight_var='weight'):
    ...
    return tf.Variable(tf.random.normal(shape=shape), shape=shape, initializer=init) * runtime_coef
...
def apply_bias_act(x, act='linear', alpha=None, gain=None, lrmul=1, bias_var='bias'):
    b = tf.Variable(tf.random.normal(shape=[x.shape[1]]), shape=[x.shape[1]], initializer=tf.initializers.zeros()) * lrmul
...
def G_main(
    ...
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
    ...
    # Setup variables.
    lod_in = tf.Variable(0.0, initializer=np.float32(0), trainable=False)
    dlatent_avg = tf.Variable(tf.random.normal(shape=[dlatent_size]), shape=[dlatent_size], initializer=tf.initializers.zeros(), trainable=False)
    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    if label_size:
        with tf.compat.v1.variable_scope('LabelConcat'):
            w = tf.Variable(tf.random.normal(shape=[label_size, latent_size]), shape=[label_size, latent_size], initializer=tf.initializers.random_normal())
...
def G_synthesis_stylegan_revised(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    lod_in = tf.cast(tf.Variable(0.0, initializer=np.float32(0), trainable=False), dtype)
    ...
    def layer(x, layer_idx, fmaps, kernel, up=False):
        ...
        noise_strength = tf.Variable(tf.random.normal(shape=[]), shape=[], initializer=tf.initializers.zeros())
    ...
    with tf.compat.v1.variable_scope('4x4'):
        with tf.compat.v1.variable_scope('Const'):
            x = tf.Variable(tf.random.normal(shape=[1, nf(1), 4, 4]), shape=[1, nf(1), 4, 4], initializer=tf.initializers.random_normal())
...
def D_stylegan(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    labels_in = tf.cast(labels_in, dtype)
    lod_in = tf.cast(tf.Variable(0.0, initializer=np.float32(0.0), trainable=False), dtype)
