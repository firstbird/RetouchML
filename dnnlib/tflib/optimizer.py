...
class Optimizer:
    ...
    def register_gradients(self, loss: TfExpression, trainable_vars: Union[List, dict]) -> None:
        ...
        with tf.name_scope(self.id + "_grad"), tf.device(device.name), tf.control_dependencies(deps):
            ...
            gate = tf.keras.optimizers.Optimizer.GATE_NONE  # disable gating to reduce memory usage
...
class SimpleAdam:
    ...
    def compute_gradients(self, loss, var_list, gate_gradients=tf.keras.optimizers.Optimizer.GATE_NONE):
        assert gate_gradients == tf.keras.optimizers.Optimizer.GATE_NONE
